...
* @subpackage	
*/
class OmhelpdeskCkClassControllerItem extends JControllerForm
{
	/**
	...
	* @var string
	*/
	protected $text_prefix = 'COM_OMHELPDESK';
	...
	*/
	public function add()
	{
		...
		$this->applyRedirection($result, array(
			'com_omhelpdesk.' . $this->view_list . '.default',
			'com_omhelpdesk.' . $this->view_item . '.' . $this->getLayout('edit')
		));
	}
	...
	*/
	protected function applyRedirection($result, $redirections, $vars = array())
	{
		...
		switch($redirection)
		{
			...
			case 'stay':
				$this->setRedirect(OmhelpdeskHelper::urlRequest($vars));
				return;
				break;
		...
		//Filters followers
		$model = CkJModel::getInstance($this->view_list, 'OmhelpdeskModel');
		if ($model)
		{
	...
	*/
	public function delete()
	{
		...
		else
		{
			...
			{
				$result = true;
				$this->setMessage(JText::plural('OMHELPDESK_ITEMS_SUCCESSFULLY_DELETED', count($cid)));
			}
			else
			{
	...
	public function redirect()
	{
		if ($this->redirect)
		{
			...
			if ($jinput->get('return') == 'json')
			{
				...
				OmhelpdeskClassAjax::responseJson(array(
	...
	*/
	protected function updateFileField($model, $fieldName, $options = array())
	{
		...
		try
		{
			$changedValue = OmhelpdeskHelperFile::updateFile($model, $fieldName, $options, $uploadDirectory);
		}
		catch(Exception $e)
		{
...
if (!class_exists('OmhelpdeskClassControllerItem')){ class OmhelpdeskClassControllerItem extends OmhelpdeskCkClassControllerItem{} }
