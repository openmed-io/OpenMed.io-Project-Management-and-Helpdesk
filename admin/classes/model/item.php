...
* @subpackage	Classes
*/
class OmhelpdeskCkClassModelItem extends JModelAdmin
{
	/**
	* Data array
	*
	...
	* @var string
	*/
	protected static $extension = 'omhelpdesk';
	...
	*/
	public function __construct($config = array())
	{
		...
		// Load the ORM system
		$this->orm = OmhelpdeskClassModelOrm::getInstance($this);
	...
	*/
	public function canCheck($record)
	{
		...
		{			
			$this->setError(JText::_("OMHELPDESK_TASK_RESULT_THE_USER_CHECKING_OUT_DOES_NOT_MATCH_THE_USER_WHO_CHECKED_OUT_THE_ITEM"));
			return false;			
		}
	...
	*/
	public function deletefiles($pks, $fileFields)
	{
		...
		$config	= JComponentHelper::getParams( 'com_omhelpdesk' );
		...
		foreach($fileFields as $fieldName => $op)
		{
			...
			foreach($files as $fileObj)
			{
				...
				if (!OmhelpdeskHelperFile::removeFile($imagePath, $op))
					$errors = true;
			}
		}
	...
	*/
	public static function fkTable($fieldname)
	{
		...
		switch($fieldname)
		{
			...
			case 'category': return $tbl.$com. 'categories';
			...
			case 'department': return $tbl.$com. 'sdepartments';	
		}
	}
	...
	*/
	...
	{
		$model = CkJModel::getInstance(ucfirst($table), 'OmhelpdeskModel');
	...
	*/
	public function getForm($data = array(), $loadData = true, $control = 'jform')
	{
		...
		$form->addRulePath(JPATH_ADMIN_OMHELPDESK . '/models/rules');
	...
	*/
	...
	{
		$modelList = CkJModel::getInstance($this->getNameList(), 'OmhelpdeskModel');
		if ($modelList)
	...
	*/
	public function integrityRestrict($relation, &$pks, $labelKey, $parentLabelKey, $editLink)
	{
		...
		foreach($pks as $id)
		{
			...
			$children = OmhelpdeskHelper::getData($childModel, array(
				'filter' => array(
			...
			if (count($children))
			{
				...
				$model = OmhelpdeskHelper::componentModel($relation->foreignModelClass, true);
				...
				$msg = sprintf(JText::_('OMHELPDESK_ERROR_IMPOSSIBLE_TO_DELETE_USED_IN'), ($itemName?$itemName:'item'), implode(', ', $childText));
	...
	*/
	protected function loadForm($name, $source = null, $options = array(), $clear = false, $xpath = false)
	{
		...
		// Get the form.
		JForm::addFormPath(JPATH_OMHELPDESK . '/models/forms');
		JForm::addFieldPath(JPATH_OMHELPDESK . '/models/fields');
	...
	*/
	public function loadRelation($name, &$item, $orm = array())
	{
		...
		$model = OmhelpdeskClassModelOrm::getModel($relation->foreignModelClass);
		if (!$model)
			return;
	...
	*/
	public function loadXref(&$item, $objectField, $xrefTable, $on, $key, $states = array(), $context = 'object.default')
	{
		...
		{
			$model = CkJModel::getInstance($xrefTable, 'omhelpdeskModel');
	...
	*/
	protected function populateState()
	{
		...
		if (defined('JDEBUG'))
			$_SESSION["Omhelpdesk"]["Model"][$this->getName()]["State"] = $this->state;
	...
	*/
	...
	{
		$baseFolder = JPATH_OMHELPDESK . '/fork/models/forms';
	...
	*/
	...
	{
		return OmhelpdeskClassModelOrm::queryDump($query, $output);
	}
	...
	*/
	public function toggle($fieldName, $pk = null, $max = 1)
	{
		...
		if (!$result)
		{
			JError::raiseWarning(1106, JText::sprintf("OMHELPDESK_MODEL_IMPOSSIBLE_TO_TOGGLE", $fieldName));
			return false;
		}
...
if (!class_exists('OmhelpdeskClassModelItem')){ class OmhelpdeskClassModelItem extends OmhelpdeskCkClassModelItem{} }
