...
* @subpackage	Classes
*/
class OmhelpdeskCkClassModelList extends JModelList
{
	/**
	* Data array
	*
	...
	*/
	...
	{
		$model = CkJModel::getInstance($this->view_item, 'OmhelpdeskModel');
	...
	*/
	public function getItems()
	{
		try
		{
			...
			{
				if (!$this->canAdmin())
					$error = JText::_('OMHELPDESK_ERROR_INVALID_QUERY');
	...
	*/
	...
	{
		if ($item)
			return CkJModel::getInstance($this->view_item, 'OmhelpdeskModel');
	...
	*/
	...
	{
		$url = 'index.php?option=com_omhelpdesk';
	...
	*/
	public function populateObjects(&$items)
	{
		...
		// Complete the object for the search plugin
		if ($this->getState('context') == 'search.plugin')
			foreach($items as $item)
				$this->populateSearchResult($item);
	}
	...
	*/
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$globalParams = JComponentHelper::getParams('com_omhelpdesk', true);
		...
		// FILTERS
			foreach($this->filter_vars as $var => $varType)
			{
				...
				{
					$date = OmhelpdeskHelperDates::timeFromFormat($value, $matches[1]);
					if ($date)
					{
	...
	*/
	public function updateXref($field, $values, $on, $id)
	{
		...
		$model = CkJModel::getInstance($this->getName(), 'OmhelpdeskModel');
		...
		else
		{
			...
			{
				$query = $db->getQuery(true);
				$query->update('#__omhelpdesk_' . $this->getName())
...
if (!class_exists('OmhelpdeskClassModelList')){ class OmhelpdeskClassModelList extends OmhelpdeskCkClassModelList{} }
