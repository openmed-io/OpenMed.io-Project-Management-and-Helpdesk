...
class OmhelpdeskCkHelperForm
{
	...
	*/
	public static function getElementGroups($element)
	{
		...
		$model = OmhelpdeskHelper::componentModel((string)$element['model'], false);
		...
		foreach ($element->children() as $option)
		{
			...
			if ($model)
			{
				...
				$group->model = (strpos($relation->model, '.')?'':'omhelpdesk.') . $relation->model;

				$model = OmhelpdeskHelper::componentModel($relation->model, false);
			}
			...
			// Label Key
			$group->alias = OmhelpdeskClassModelOrm::tableFieldAlias($group->on, false);
	...
	*/
	public static function getModel($field, $item = false)
	{
		...
		return OmhelpdeskHelper::componentModel($model, $item);
	}
...
if (!class_exists('OmhelpdeskHelperForm')){ class OmhelpdeskHelperForm extends OmhelpdeskCkHelperForm{} }
