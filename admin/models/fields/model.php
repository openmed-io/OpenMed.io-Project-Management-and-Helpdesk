...
* @subpackage	Form
*/
class OmhelpdeskCkFormFieldModel extends JFormFieldGroupedList
{
	/**
	* Foreign Key Filter. 
	*
	...
	*/
	protected function getGroups()
	{
		...
		$listGroups = OmhelpdeskHelperForm::getElementGroups($this->element);
		...
		$items = OmhelpdeskHelperForm::getItems($this);
		...
		foreach($items as $item)
		{
			...
			{
				$val = OmhelpdeskHelper::parseValue($item, $labelKey);
				if ($val !== null)
	...
	*/
	protected function getOptions()
	{
		...
		if (!empty($items))
		foreach ($items as $item)
			$options[$item->$listKey] = OmhelpdeskHelper::parseValue($item, $labelKey);
	...
	*/
	public function prepareModel(&$model)
	{
		...
		$groups = OmhelpdeskHelperForm::getElementGroups($this->element);
...
if (!class_exists('JFormFieldModel')){ class JFormFieldModel extends OmhelpdeskCkFormFieldModel{} }
