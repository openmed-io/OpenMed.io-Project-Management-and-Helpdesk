...
* @subpackage	Form
*/
class OmhelpdeskCkFormRuleNumeric extends OmhelpdeskClassFormRule
{
	/**
	...
	*/
	public function getJsonRule($field)
	{
		...
		else if ($numericFormat = $field->getAttribute('numericFormat'))
		{
			...
			$alertText = JText::sprintf('OMHELPDESK_ERROR_INCORRECT_FORMAT_EXPECTED_DIGITS_MAX_DECIMALS_MAX', $length, $scale);
		}
		else
			// Fallback default error message for all numeric types
			$alertText = JText::sprintf('OMHELPDESK_ERROR_EXPECTED_NUMERIC_FORMAT');
...
if (!class_exists('JFormRuleNumeric')){ class JFormRuleNumeric extends OmhelpdeskCkFormRuleNumeric{} }
