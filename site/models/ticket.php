...
* @subpackage	Classes
*/
class OmhelpdeskCkModelTicket extends OmhelpdeskClassModelItem
{
	/**
	...
	*/
	public function getLayout()
	{
		...
		return $jinput->get('layout', 'ticket', 'STRING');
	}
	...
	*/
	public function getTable($type = 'ticket', $prefix = 'OmhelpdeskTable', $config = array())
	{
	...
	*/
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_omhelpdesk.edit.ticket.data', array());
		...
		if (empty($data)) {
			...
			if ($this->getState('ticket.id') == 0)
			{
				...
				$data->sprint = $jinput->get('filter_sprint', $this->getState('filter.sprint'), 'INT');
				$data->pilot = $jinput->get('filter_pilot', $this->getState('filter.pilot'), 'INT');
				$data->requester = $jinput->get('filter_requester', $this->getState('filter.requester'), 'INT');
				$data->description = null;
				$data->attachment = null;
				$data->hours_of_work = null;
				$data->overtime = 0;
				$data->finish_date_n_time = null;
	...
	* @param	integer	$pk	The primary id key of the ticket
	*
	* @return	void
	*/
...
if (!class_exists('OmhelpdeskModelTicket')){ class OmhelpdeskModelTicket extends OmhelpdeskCkModelTicket{} }
